...
export default function PdfSplitScreen() {
  ...
  const pick = async () => {
    ...
    if (!r.canceled && r.assets[0]) { setFile({ name: r.assets[0].name, size: r.assets[0].size ?? 2 * 1048576 }); setDone(false); Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium); }
  };
      ...
      <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        ...
        {file && (
          <>
            ...
            <TouchableOpacity onPress={split} disabled={loading} style={[styles.btn, { backgroundColor: done ? "#10B981" : ACCENT, borderRadius: colors.radius, marginHorizontal: 16 }]}>
              {loading ? <ActivityIndicator color="#FFF" /> : (
                <>
                  <MaterialCommunityIcons name={done ? "check-circle" : "call-split"} size={20} color="#FFF" />
                  <Text style={styles.btnTxt}>{done ? "Split Complete!" : "Split PDF"}</Text>
                </>
              )}
            </TouchableOpacity>
          </>
